# Fedora 42 Encrypted Installation with Ventoy USB Boot
...
**Related:** See [README.md](README.md) for backup/restore scripts to protect against losing your boot USB.
...
Example fstab mounts (from `/etc/fstab`):
```
...
## Manual Recovery (Without Scripts)

If you lose your boot USB and don't have a backup from `backup-boot.sh`, you can manually recreate the bootloader.

### What you need:
- New USB drive
- Any Linux live ISO (Fedora, Ubuntu, etc.)

### Option A: Full Manual Chroot Recovery
...
### Option B: Quick Data Access (No Full Recovery)

If you just need to access your files:
...
For a simple boot USB without Ventoy:
...
- **Backup the USB** — use `backup-boot.sh` regularly, or create a second boot USB
